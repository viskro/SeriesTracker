...
export async function getFavorisData() {
	...
	const transformedUserShows = userShows.map((show) => {
		const lastEpisode = lastWatchedEpisodes.find(
			(ep) => ep.showId === show.id_show
		);
		const totalEpisodes = show.shows.seasons.reduce(
			(acc: number, season) => acc + season.episodes.length,
			0
		);
